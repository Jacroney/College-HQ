...
2. **Find your profile Lambda function** (the one handling your profile endpoints)
3. **Replace the entire function code** with the content below
...
exports.handler = async (event) => {
  ...
  try {
    ...
    // Extract and validate Cognito JWT token
    ...
    // Check if this is a course lookup request
...
// Get courses for a specific university and major
...
// Get student profile with auto-creation
async function getProfile(userId, userInfo) {
  try {
    ...
    if (!result.Item) {
      ...
      const newProfile = createProfileFromCognito(userId, userInfo);
...
// Create profile from Cognito user information
...
// Transform DynamoDB data to frontend format
...
**After pasting this code, click "Deploy" in the Lambda console.**